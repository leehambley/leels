...
impl Stepper {
    pub fn new(cfg: StepperConfig) -> Self {
        ...
        Self { cfg, pos: 0, dir: None, speed: start_speed, rem: 0, next_due_us: 0, last_step_us: None }
    }
    ...
    pub fn poll(&mut self, now_us: u64, target: Target) -> Action {
        ...
        if let Some(last) = self.last_step_us {
            ...
            if gap > 2 * interval {
                ...
                self.speed = self.speed.min(gap_speed).max(self.cfg.start_speed);
            }
        }
        ...
        self.speed = if braking {
            self.speed.saturating_sub(dv).max(self.cfg.start_speed)
        } else {
            (self.speed + dv).min(self.cfg.max_speed)
        };
